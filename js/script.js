...
if (popupLinks.length > 0) {                                                //проверяем есть ли у нас ссылки popupLinks
...
function popupOpen(currentPopup) {
   if (currentPopup && unlock) {                                     //проверяем есть ли popup и проверяем открыт ли он
      ...
      currentPopup.addEventListener("click", function (e) {
         if (!e.target.closest('.popup__content')) {                 //отсекаем все, кроме области за popup, если у нажатого объекта нету родителя с классом popup__content
...
function popupClose(popupActive, doUnlock = true) {    //передаем открытый объект
   if (unlock) {
...
function bodyLock() {
   const lockPaddingValue = window.innerWidth - document.querySelector('.wrapper').offsetWidth + 'px'; //высчитываем разницу между шириной viewport и шириной объекта внутри него
